            ...
            "${path.module}/hostvars_template.yml",
                {
                    ...
                    juiceshop_public_ip_1  = azurerm_public_ip.juiceshop_public_ip[0].ip_address
                    juiceshop_public_ip_2  = azurerm_public_ip.juiceshop_public_ip[1].ip_address
                    grafana_virtual_ip     = azurerm_network_interface.ext-nic[count.index].private_ip_addresses[2]
                    demo_virtual_ip        = azurerm_network_interface.ext-nic[count.index].private_ip_addresses[3]
                    azure_log_id           = azurerm_log_analytics_workspace.azure_telemetry.workspace_id
                    azure_log_key          = azurerm_log_analytics_workspace.azure_telemetry.primary_shared_key
